...
class FastSAM_with_realsense(object):
  ...
  def image_cb( self, image_msg, info_msg, depth_msg):
    try:
        ...
        # 後面fastsam再做mask濾除時會使用(depth_useless_mask)
        # 給450～500 就差不多了(原先是1000)
    ...
    # 将OpenCV图像转换为PIL图像（這邊可以凱成是否要用過濾過得img)(self.image_rgb or self.image_masked)
    ...
    # # 先建立全白的mask. 只要有的mask就把該pixel變成黑色
    ...
    if (self.number_of_result == 5):
        ...
        # 将概率大于80%的像素位置设置为黑色
        final_result[pixel_probabilities > 0.8] = [0, 0, 0]
