...
bool start_demo_flag = false; 
bool start_handover_flag = false; 
...
void start_demo(const std_msgs::Bool msg)
{
	start_handover_flag = true;
	
	ROS_INFO("start demo!!!!!!!!!!!!!!!!!!!!!!!!!");

}

void start_handover(const std_msgs::Int16 msg)
{
	start_demo_flag = true;
	
	ROS_INFO("start demo11111111111111111111111111");
...
bool state3_flag = false;

void set_start_flag(const std_msgs::String::ConstPtr& msg)
{
	state3_flag = true;
}

bool check_arrive(std::vector<double> target_angle)
{	
	double achieve_err = 0.0;
	double err_temp = 0.0;
	bool arrived_flag = false;
	for(int i = 0; i < NUMBER_OF_DOFS; i++)
	{
		err_temp = err_temp + (g_robot_joint_angle[i] - target_angle[i]) * (g_robot_joint_angle[i] - target_angle[i]);
	}
	achieve_err = sqrt(err_temp);
	std::cout<<achieve_err<<std::endl;
	if (achieve_err < 1e-4)
	{
		arrived_flag = true;
		std::cout<<"yaaaaaaaaaaaaaa"<<std::endl;
...
int main(int argc, char **argv)
{
	...
	// script_client 為global 的ServiceClient
	...
	armHere = node_handle.advertise<std_msgs::Int16>("armHere",10);
	spinner.start();

	ros::Subscriber sub_demo_start  = node_handle.subscribe("/handover", 1,start_demo);
	ros::Subscriber sub_handover_start  = node_handle.subscribe("/AMMArrived", 1,start_handover);

	// use to start this code
	ros::Subscriber sub_start_step3 = node_handle.subscribe("/start_step3", 100, set_start_flag);

	
	// target_pose type is tm_msgs::SetPositions (global)
	// 用來吃main 裡面給的特定位置
	target_pose.request.motion_type = tm_msgs::SetPositions::Request::PTP_J;
	target_pose.request.velocity = 6.0;//rad/s
	target_pose.request.acc_time = 0.1;
	target_pose.request.blend_percentage = 0;
	target_pose.request.fine_goal = false;

	// // target_pose_line type is tm_msgs::SetPositions (global)
	...
	// script_srv type is tm_msgs::SendScript
    ...
    if (script_srv.response.ok) ROS_INFO_STREAM("Set spdmodeon to robot");
		else ROS_WARN_STREAM("Set spdmodeon to robot , but response not yet ok ");
	}else
	{
		ROS_ERROR_STREAM("Error Set spdmodeon to robot");
	}


	

	// demo_home position setting (position control)
	std::vector<double> demo_pick_pos = { 0.314837, -0.353855, 2.45941, -0.381694, 1.67354, 0.348203 };
	std::vector<double> demo_after_pick= { 0.371119, -0.382808, 2.1203, -0.164575, 1.54113, 0.346933 };
	std::vector<double> demo_home_position = { -0.00171694, -0.771476, 2.00296, -0.23942, 1.68463, -0.0785592 };
	// picking position setting (position control)
	std::vector<double> pick_position = { 0.297363, -0.244882, 2.49836, -0.727114, 1.69758, -0.558088 };
	// handover position setting (position control)
	std::vector<double> handover_position = {  0.0764869,    -0.00104512,    1.7449,    -1.01787,    1.54505,    -0.0399196 };
	std::vector<double> handover_preposition = {  0.179481,    0.335416,    1.84223,    -2.20783,    1.41848,    0.0480935 };
	// std::vector<double> handover_position_new = {  0.179108,  0.433235,    1.26857,    -1.86633,    1.48297,    -0.0176569 };
	std::vector<double> handover_position_new = {  0.0763438,  -0.120177,    1.78218,    -1.3947,    1.53341,    0.000242407 };
	...
	// // go to demo_after_pick 
	// target_pose.request.positions.clear();
	// target_pose.request.positions = demo_after_pick;
	...
	// 	return 1;
	// }
	// while(!check_arrive(demo_after_pick))
	// {
	// 	print_info("approaching to the target");
	// }
	// sleep(1);
	// print_info("go to picking position !!!!!!!!!!!!!!!!");


	// // go to picking position
	// target_pose.request.positions.clear();
	// target_pose.request.positions = demo_pick_pos;
	// if (pos_client.call(target_pose))                             
	// {
	// 	if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
	// 	else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
	...
	// 	return 1;
	// }
	// while(!check_arrive(demo_pick_pos))
	// {
	// 	print_info("approaching to the target");
	// }
	// sleep(1);
	...
	// go to handover_preposition 
	target_pose.request.positions.clear();
	target_pose.request.positions = handover_preposition;
	if (pos_client.call(target_pose))                             
	{
		if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
		else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
	...
	while(!check_arrive(handover_preposition))
	{
		print_info("approaching to the target");
	}
	sleep(1);
	print_info("ggggggggggggggggggggggggggggggggg");
	////Close
	// set_gripper(1);
	// pub_ctrl.publish(gripper_command);
	// sleep(1); //應該1秒就可以
	...
	// /////////////////////////////// go back to demo home
	// target_pose.request.positions.clear();
	// target_pose.request.positions = demo_home_position;
	// if (pos_client.call(target_pose))                             
	// {
	// 	if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
	// 	else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
	...
	// 	return 1;
	// }
	// while((!check_arrive(demo_home_position)) && ros::ok())
	// {
	// 	print_info("11111 approaching to the target");
	// }
	// sleep(1);
	// // sleep(3);
	// print_info("go to demo_home_position  222222222222222222222222222222!!!!!!!!!!!!!!!!");
	...
	std_msgs::Int16 data;
	data.data = 0;
	armHere.publish(data);
	print_info("ggggggggggggggggggggggggggggggggg");


	bool finish_flag = false;
	while(ros::ok() && !start_handover_flag)
	{
		loop_rate.sleep();
		ros::spinOnce();
	} 
	while (ros::ok() && start_demo_flag)
	{
		...
		// 	if (pos_client.call(target_pose))                             
		// 	{
		// 		if (target_pose.response.ok) ROS_INFO_STREAM("check_arrive!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
		// 		else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
		// 	}
		// 	else
		// 	{
		...
		// while(ros::ok() &&!check_arrive(handover_preposition))
		// {
		// 	print_info("approaching to the target");
		// 	if (pos_client.call(target_pose))                             
		// 	{
		...
		// 	ros::spinOnce();
		// }
		// print_info("go to handover_preposition !!!!!!!!!!!!!!!!");
		...
		target_pose.request.positions.clear();
		target_pose.request.positions = handover_position_new;
		if (pos_client.call(target_pose))                             
		{
			if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
			else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
		...
		while(!check_arrive(handover_position_new))
		{
			print_info("approaching to the target");
		}
		
		print_info("go to handover_position_new !!!!!!!!!!!!!!!!");
		ros::spinOnce();

		
		

		// open gripper, wait for 3 sec
		sleep(1.0);
		set_gripper(3);
		pub_ctrl.publish(gripper_command);
		print_info("open gripper");
		sleep(2);
