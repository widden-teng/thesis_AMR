...
{
	ros::init(argc, argv, "go_certain_place");
	...
	// handover home
	std::vector<double> handover_home = { 0.115866, -0.879447, 1.95442, -0.529465, 1.47669, -0.00489662 };
	...
	target_pose.request.positions = handover_home;
	...
	while((!check_arrive(handover_home)) && ros::ok())
	{
		print_info("11111 approaching to the target");
	}
	sleep(1);
	// sleep(3);
	print_info("go to handover_home  222222222222222222222222222222!!!!!!!!!!!!!!!!");
	...
	while (false)
	{
		...
		if (state3_flag){
			...
			target_pose.request.positions.clear();
			target_pose.request.positions = pick_position;
			if (pos_client.call(target_pose))                             
            {
                if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
                else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
			...
			while(!check_arrive(pick_position))
			{
			...
			while(!check_arrive(demo_home_position))
			{
				print_info("approaching to the target");
			}
			sleep(1);
			print_info("go to demo_home_position  !!!!!!!!!!!!!!!!");

			// go to handover position
			target_pose.request.positions.clear();
			target_pose.request.positions = handover_position;
			if (pos_client.call(target_pose))                             
            {
                if (target_pose.response.ok) ROS_INFO_STREAM("SetPositions to robot");
                else ROS_WARN_STREAM("SetPositions to robot , but response not yet ok ");
			...
			while(!check_arrive(handover_position))
			{
				print_info("approaching to the target");
			}
			sleep(1);
			print_info("go to handover position !!!!!!!!!!!!!!!!");
