...
public:
	...
	Tag_Pose(){
		...
		// tf::Vector3 v_robot2tag0(0.5175511108398437, 0.16396395263671874, 0.001284 );
		...
		tf::Vector3 v_tag1_to_ARbase(0.2466,-0.03,0.0);
	...
	void target_Callback(const geometry_msgs::PoseStamped::ConstPtr& TargetPose)
	{
		...
		tf::Quaternion q_target2tool(0.0, 0.0 ,1.0, 0.0);
		tf::Vector3 v_target2tool(0.0, 0.0, -0.265);
	...
	void hole_target_set(){
		...
		tf::Vector3 v_hole2target(0.0, 0.0, 0.27);
