...
#define REPEATABILITY 0.01 //0.01//0.00005 //TODO: modify attractive force

#define GRIPPER_LENGTH 0//0.205 //0.235
...
bool Hexagon_Demo()
{
    ...
    std::vector<double> InitialPosition = {0.3343,    -0.4224,    2.3233,    -0.3237,    1.5618,    0.3197};
    std::vector<double> FinalPosition = {-0.2481,    0.7712,    1.2664,    -0.4671,    1.5567,    -0.2638};
...
int main(int argc, char **argv)
{
    ...
    while (ros::ok())
    {
        ...
        {
            std::vector<double> vec1 = {0.344,    -0.5466,    2.0456,    0.0747,    1.5593,    0.334};
        ...
        {
            std::vector<double> vec1 = {-0.3186,    0.1422,    1.4481,    -1.5521,    1.4309,    0.0006};
        ...
        else if(strncmp(cstr, "wbox", 4) == 0)
        {
            std::vector<double> vec1 = {-0.5084,    0.4679,    1.2283,    -0.7621,    1.8322,    -0.4725};
            ...
            print_info("Back to position white box pos");
        }    
        ...
        else if (strncmp(cstr, "goReverse", 9) == 0)
        {
           unsigned char ch = 0;
           std_msgs::Int32 reset_local_replan_msg;
           reset_local_replan_msg.data = 1;
        ...
        else if (strncmp(cstr, "demo", 4) == 0)
        {
           ...
           reset_local_replan_msg.data = 1;
            ...
            std::vector<double> start_pose = {0.3343,    -0.4224,    2.3233,    -0.3237,    1.5618,    0.3197};
            std::vector<double> goal_pose = {-0.2481,    0.7712,    1.2664,    -0.4671,    1.5567,    -0.2638};
            ...
            for(int i = 0; i < 2; i++){
                ...
                Hexagon_Demo();
                // pos_srv.request.motion_type = 1;
                // pos_srv.request.positions = goal_pose;
                // pos_srv.request.velocity = 30;
                // pos_srv.request.acc_time = 0.2;
                // pos_srv.request.blend_percentage = 0;
                // pos_srv.request.fine_goal = false;
                ...
                Hexagon_Demo_Reverse();
                // pos_srv.request.motion_type = 1;
                // pos_srv.request.positions = start_pose;
                // pos_srv.request.velocity = 30;
                // pos_srv.request.acc_time = 0.2;
                // pos_srv.request.blend_percentage = 0;
                // pos_srv.request.fine_goal = false;
        ...
        // else if(strncmp(cstr, "readPath", 8) == 0)
        // {
        //     read_static_path();
        // }
        else
