...
public:
	...
	Tag_Pose(){
		...
		//tf::Vector3 v_robot2tag0(0.50901, 0.16206, 0.0051718);
		tf::Vector3 v_robot2tag0(0.50731, 0.16036, 0.0055018);
  ...
  //   	q_qr_to_cad.setRPY(-0.236 *M_PI/180,  -0.5987*M_PI/180,  -89.2079*M_PI/180);
		...
		//0810
		// pos=[-0.0391764000000000,0.168282000000000,0.0726738000000000]
		// rot=[89.3691506185081	-1.22633577904859	0.183852081491121]
		tf::Quaternion q_cad2qr;
		q_cad2qr.setRPY(0.183852 *M_PI/180,  -1.226336*M_PI/180,  89.36915*M_PI/180);
		tf::Vector3 v_cad2qr(-0.0391764,0.168282,0.0726738);
		...
		tf::Matrix3x3 m(T_qr_to_cad.getRotation());
	    m.getRPY(tm_target_p[3], tm_target_p[4], tm_target_p[5]);

	    //ROS_INFO("targetA = %f %f %f %f %f %f\n",T_qr_to_cad.getOrigin().x(),T_qr_to_cad.getOrigin().y(),T_qr_to_cad.getOrigin().z(),tm_target_p[3]*180/M_PI,tm_target_p[4]*180/M_PI,tm_target_p[5]*180/M_PI);
		...
		// sub_target=nh.subscribe("/targetA", 10, &Tag_Pose::target_Callback_qr,this);
		...
		robot_states=nh.subscribe("/feedback_states", 10, &Tag_Pose::Robot_Callback, this);

	}
	void Robot_Callback(const tm_msgs::FeedbackState::ConstPtr& robotPose){
			tf::Quaternion q(0,0,0,1);
			tf::Vector3 vec(robotPose->tool_pose[0],robotPose->tool_pose[1],robotPose->tool_pose[2]);
			q.setRPY(robotPose->tool_pose[3],robotPose->tool_pose[4],robotPose->tool_pose[5]);
			TM5_pose.setOrigin(vec);
			TM5_pose.setRotation(q);
	...
	void loop_confirm(geometry_msgs::Transform& msg_robottarget){
		...
		tf::Vector3 v_target2tool(-0.00, -0.00, 0.266);
       ...
       // 		}
			...
			T_robot_traj = T_target2tool*TM5_pose.inverse()*T_robot_to_tag0*T_cam_to_tag0.inverse()*T_cam_to_qr*T_qr_to_cad;
			...
			T_robot_traj.setRotation(q2);
	   		...
	   		float x = pow(msg_robottarget.translation.x - TM5_pose.getOrigin().x(), 2);
	   		//ROS_WARN("%f", x);
	   		float y = pow(msg_robottarget.translation.y - TM5_pose.getOrigin().y(), 2);
	   		float z = pow(msg_robottarget.translation.z - TM5_pose.getOrigin().z(), 2);
	   		//float rx = pow(msg_robottarget.rotation.x - TM5_pose.getRotation().x(), 2);
	   		//float ry = pow(msg_robottarget.rotation.y - TM5_pose.getRotation().y(), 2);
	   		//float rz = pow(msg_robottarget.rotation.z - TM5_pose.getRotation().z(), 2);
	   		//float rw = pow(msg_robottarget.rotation.w - TM5_pose.getRotation().w(), 2);
	...
	void target_Callback_qr(const geometry_msgs::PoseStamped::ConstPtr& TargetPose)
	{
		...
		q_target.setRPY(-0.0 *M_PI/180,  0.0*M_PI/180,  0*M_PI/180);//old
	...
	void target_Callback_qr_ar(const geometry_msgs::PoseStamped::ConstPtr& TargetPose)
	{
	    ...
	    tf::Quaternion q_target;
        q_target.setRPY(-0.5 *M_PI/180,  0.8*M_PI/180,  0*M_PI/180);//multi
