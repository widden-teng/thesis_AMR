...
### approach variables 
EFF_TOOL_DISTANCE = 0.006 + 0.063 + 0.175 # origin+mine+gripper
HANDOVER_DIS = EFF_TOOL_DISTANCE + 0.05  #0.29
LAST_TARGET_DIS = HANDOVER_DIS + 0.05 #0.34
# LAST_TARGET_DIS = 0.6 #testing
...
# ###gesture ###
# base_options_gesture = python.BaseOptions(model_asset_path='/home/tm5/widden/src/test_pkg/scripts/weight/gesture_recognizer0924.task')
...
###landmarks ###
base_options_landmark = python.BaseOptions(model_asset_path='/home/tm5/widden/src/test_pkg/scripts/weight/hand_landmarker.task',delegate=python.BaseOptions.Delegate.GPU)
...
class handpose_mediapipe(object):
  def __init__(self):
    ...
    self.image_sub = message_filters.Subscriber('/top_camera/color/image_raw', Image)
    self.camera_info_sub = message_filters.Subscriber('/top_camera/color/camera_info', CameraInfo)
    self.depth_sub = message_filters.Subscriber('/top_camera/aligned_depth_to_color/image_raw', Image)
    ...
    #### Pub
    self.xyz_vector_pub = rospy.Publisher("/top_camera/handpose_cameralink",PoseArray, queue_size=1)
    self.mediapipe_img_pub = rospy.Publisher("/top_camera/annotated_image", Image, queue_size = 1)
    #用來UKF更換發tf的通知(看手掌是否距離camera過近)
    self.stop_update_target_pub = rospy.Publisher("/top_camera/stop_update_target_flag",Bool, queue_size=1)
    # self.gesture_pub = rospy.Publisher('/top_camera/gesture_topic', String, queue_size=1)

    ### for stop update target flag
    self.stop_update_target_bool_msg = Bool()
    self.stop_update_target_bool_msg.data = False
...
def main():
  rospy.init_node('top_camera_demo_handpose', anonymous=True)
