...
class Pose_adjust_tf{
    ...
    private:
        ros::NodeHandle n;
        ros::Subscriber top_handpose_sub, side_handpose_sub,fuse_sub, start_record_sub, top_noupdate_sub;
        ...
        tf::TransformBroadcaster odom_broadcaster, base_link_broadcaster, side2top_broadcaster, top_broadcaster;
...
Pose_adjust_tf::Pose_adjust_tf()
{
    ...
    // 用來判斷是否還要根據top 來發布tf( handpose_base_link&handpose_odom), 可關掉(把side的部份註解掉)
...
void Pose_adjust_tf::side_camera_cb(const geometry_msgs::PoseArray::ConstPtr& side_hand_psa){
    ...
    side2top_broadcaster.sendTransform(tf::StampedTransform(side_tf, ros::Time::now(), "side_camera_link", "side_handpose"));
    ...
    try{
        ...
        side2top_broadcaster.sendTransform(tf::StampedTransform(side2top_tf, odo_msg_side_camera.header.stamp, "top_camera_link", "side2top_handpose"));
