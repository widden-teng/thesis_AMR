...
###
# top+side (no校正工具)
EFF_TOOL_DISTANCE = 0.245
HANDOVER_DIS = EFF_TOOL_DISTANCE + 0.05
LAST_TARGET_DIS = HANDOVER_DIS + 0.05
######
...
class handpose_mediapipe(object):
  def __init__(self):
    ...
    self.mediapipe_img_pub = rospy.Publisher("/camera/annotated_image", Image, queue_size = 1)
    #用來UKF更換發tf的通知(看手掌是否距離camera過近)
    self.stop_update_target_pub = rospy.Publisher("/camera/stop_update_target_flag",Bool, queue_size=1)
    # self.gesture_pub = rospy.Publisher('/camera/gesture_topic', String, queue_size=1)
